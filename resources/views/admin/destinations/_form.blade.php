            ...
            <div id="tab-basic" class="tab-content space-y-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', isset($destination) ? $destination->title : '') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="title_card" id="title_card" value="{{ old('title_card', isset($destination) ? $destination->title_card : '') }}"
                            class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition duration-200 bg-gray-50/50 hover:bg-gray-50 focus:bg-white"
                            placeholder="Ex: Gramado">
                        @error('title_card')
                            <p class="text-red-500 text-xs mt-1 font-medium">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="text" name="category" id="category" value="{{ old('category', isset($destination) ? $destination->category : '') }}"
                            class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition duration-200 bg-gray-50/50 hover:bg-gray-50 focus:bg-white"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.01" name="price" id="price" value="{{ old('price', isset($destination) ? $destination->price : '') }}"
                    ...
                    <input type="hidden" name="type" id="pacotes" value="{{ $tripType ?? 'viagem-em-grupo' }}">
                </div>
                                ...
                                <div class="p-5 border border-slate-100 rounded-xl bg-white testimonial-row space-y-4 shadow-sm relative animate-fade-in">
                                    ...
                                    <div class="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <select name="testimonials[{{ $index }}][rating]" class="w-full px-3.5 py-2.5 rounded-lg border border-gray-200 focus:border-[#001c3d] text-xs font-bold focus:outline-none bg-slate-50/30 focus:bg-white cursor-pointer">
                                        ...
                                        <div>
                                            ...
                                            <select name="testimonials[{{ $index }}][is_active]" class="w-full px-3.5 py-2.5 rounded-lg border border-gray-200 focus:border-[#001c3d] text-xs font-bold focus:outline-none bg-slate-50/30 focus:bg-white cursor-pointer">
                                    ...
                                    <div>
                                        ...
                                        <textarea name="testimonials[{{ $index }}][content]" rows="3" placeholder="Escreva o depoimento..."
                                            class="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 focus:border-[#001c3d] text-xs font-semibold focus:outline-none transition-colors bg-slate-50/30 focus:bg-white">{{ old("testimonials.{$index}.content", $testimonial->content) }}</textarea>
                                    </div>
                                </div>
            ...
            <div id="tab-details" class="tab-content hidden space-y-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="duration" id="duration" value="{{ old('duration', isset($destination) ? $destination->duration : '') }}"
                            class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition duration-200 bg-gray-50/50 hover:bg-gray-50 focus:bg-white"
                            placeholder="Ex: 7">
                        @error('duration')
                            <p class="text-red-500 text-xs mt-1 font-medium">{{ $message }}</p>
                            ...
                            <div class="rounded-2xl border border-gray-100 bg-white shadow-[0_4px_20px_rgba(0,0,0,0.01)] overflow-hidden itinerary-day-row transition-all duration-300 hover:border-gray-200" data-day-index="{{ $dayIndex }}">
                                ...
                                <div class="p-5 space-y-5">
                                    <div class="grid grid-cols-1 sm:grid-cols-12 gap-4">
                                        <div class="sm:col-span-2">
                                            ...
                                            <input type="number" name="itinerary[{{ $dayIndex }}][day_number]" value="{{ isset($day) ? $day->day_number : '' }}" placeholder="Ex: 1"
                                                class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 text-xs font-bold text-center focus:outline-none transition-all bg-slate-50/30 focus:bg-white shadow-inner">
                                        </div>
                                    ...
                                    <div class="bg-slate-50 p-5 rounded-xl border border-gray-200/60 shadow-inner">
                                            ...
                                            @foreach($day->activities as $actIndex => $activity)
                                                <div class="flex gap-2 items-center activity-row bg-white p-2 pl-3 rounded-xl border border-gray-200 shadow-[0_2px_10px_rgba(0,0,0,0.01)] transition-all focus-within:border-[#001c3d] focus-within:ring-2 focus-within:ring-[#001c3d]/5">
                                                    ...
                                                    <input type="text" name="itinerary[{{ $dayIndex }}][activities][]" value="{{ isset($activity) ? $activity->activity : '' }}" placeholder="Descreva a atividade do dia..."
                                                        class="flex-1 border-0 focus:ring-0 px-2 py-1.5 text-xs font-medium focus:outline-none text-gray-700 placeholder-gray-400">
                                        ...
                                        <button type="button" onclick="addActivityRow(this)" class="mt-4 inline-flex items-center gap-2 text-[#001c3d] hover:text-[#001126] text-[10px] font-extrabold uppercase tracking-wider bg-white px-3 py-2 rounded-lg border border-gray-200 shadow-sm hover:border-gray-300 transition-all">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Se existirem erros de validação retornados pelo Laravel, troca para a tab do primeiro campo com erro
    const firstErrorField = form.querySelector('.border-red-500, [name].is-invalid') || document.querySelector('.text-red-500');
    if (firstErrorField) {
...
// Contadores para garantir índices únicos dinâmicos no formulário
let includeIndexCounter = document.querySelectorAll('.include-row').length;
let highlightIndexCounter = document.querySelectorAll('.highlight-row').length;
let itineraryIndexCounter = document.querySelectorAll('.itinerary-day-row').length;
let observationIndexCounter = document.querySelectorAll('.observation-row').length;
let testimonialIndexCounter = document.querySelectorAll('.testimonial-row').length;
...
function addIncludeRow() {
    ...
    const index = includeIndexCounter++;
    const html = `
        <div class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center include-row bg-white p-4 rounded-xl border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.01)] hover:border-gray-200 transition-all group">
            <div class="relative shrink-0 sm:w-44">
                <select name="includes[${index}][type]" class="w-full bg-slate-50 border border-gray-200 rounded-xl px-3 py-2.5 text-xs font-bold text-gray-700 focus:outline-none focus:border-[#001c3d] focus:bg-white transition-colors cursor-pointer appearance-none">
            ...
            <div class="flex-1">
                <input type="text" name="includes[${index}][text]" placeholder="Ex: Passagem de ida e volta em classe econômica"
                    class="w-full bg-slate-50/50 focus:bg-white border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 px-4 py-2.5 rounded-xl text-xs font-medium focus:outline-none transition-all shadow-inner">
            </div>
...
function addHighlightRow() {
    ...
    const index = highlightIndexCounter++;
    const html = `
        <div class="rounded-2xl border border-gray-100 bg-white shadow-[0_4px_20px_rgba(0,0,0,0.01)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.02)] overflow-hidden highlight-row transition-all duration-300 flex flex-col justify-between group hover:border-gray-200">
...
function addItineraryDayRow() {
    ...
    const index = itineraryIndexCounter++;
    const html = `
...
function addActivityRow(btn) {
    const dayRow = btn.closest('.itinerary-day-row');
    const dayIndex = dayRow.getAttribute('data-day-index');
    const container = dayRow.querySelector('.activities-container');
...
function addObservationRow() {
    ...
    const index = observationIndexCounter++;
    const html = `
        <div class="observation-row flex gap-3 items-start bg-amber-50/40 border border-amber-100 rounded-xl p-3">
...
function addTestimonialRow() {
    ...
    const index = testimonialIndexCounter++;
    const html = `
        <div class="p-5 border border-slate-100 rounded-xl bg-white testimonial-row space-y-4 shadow-sm relative animate-fade-in">
            ...
            <div class="space-y-3">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <select name="testimonials[${index}][rating]" class="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 focus:border-[#001c3d] text-xs focus:outline-none bg-slate-50/30">
                    ...
                    <div>
                        ...
                        <select name="testimonials[${index}][is_active]" class="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 focus:border-[#001c3d] text-xs focus:outline-none bg-slate-50/30">
                ...
                <div>
                    ...
                    <textarea name="testimonials[${index}][content]" rows="3" placeholder="Escreva o depoimento..."
                        class="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 focus:border-[#001c3d] text-xs focus:outline-none transition-colors bg-slate-50/30"></textarea>
                </div>
            </div>
            <button type="button" onclick="removeRow(this)" class="absolute top-2 right-2 p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                <i class="fas fa-trash-alt text-sm"></i>
...
document.addEventListener('DOMContentLoaded', function () {
    // ... (mantenha o seu código de client-side validation existente aqui) ...

    // --- CÓDIGO NOVO PARA LIDAR COM ERROS DO LARAVEL ---
    @if($errors->any())
        // Encontra o primeiro texto de erro que o Laravel cuspiu na tela
        const firstErrorElement = document.querySelector('p.text-red-500');
        
        if (firstErrorElement) {
            // Descobre em qual aba esse erro está
            const tabContent = firstErrorElement.closest('.tab-content');
            
            if (tabContent) {
                // Pega o ID da aba (ex: 'tab-details' vira 'details')
                let tabId = tabContent.id.replace('tab-', '');
                
                // Abre a aba automaticamente
                switchTab(null, tabId);
                
                // Faz o scroll suave até o campo com erro
                setTimeout(() => {
                    firstErrorElement.closest('div').scrollIntoView({ behavior: 'smooth', block: 'center' });
                }, 300);
            }
        }
    @endif
