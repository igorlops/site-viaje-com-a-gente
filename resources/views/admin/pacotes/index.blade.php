...
@section('page_title', 'Gerenciar Pacotes')

@section('admin_content')
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <p class="text-sm text-gray-500">Cadastre e altere os pacotes prontos exibidos no site.</p>
        <a href="{{ route('admin.pacotes.create') }}" class="inline-flex items-center gap-2 bg-[#109e4a] hover:bg-[#0d9648] text-white px-5 py-3 rounded-lg font-bold text-xs uppercase tracking-wider transition duration-200 shadow-sm">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-6 border-b border-gray-200">
            <h2 class="font-bold text-gray-800">Lista de Pacotes</h2>
        </div>
                        ...
                        <tr class="hover:bg-gray-50 transition duration-150">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <form action="{{ route('admin.pacotes.duplicate', $dest->id) }}" method="POST" class="inline">
                                        @csrf
                                    ...
                                    <a href="{{ route('admin.pacotes.edit', $dest->id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition duration-200" title="Editar">
                                        <i class="fas fa-edit text-xs"></i>
                                    </a>
                                    
                                    <form action="{{ route('admin.pacotes.destroy', $dest->id) }}" method="POST" onsubmit="return confirm('Deseja realmente excluir este pacote?')">
                                        @csrf
                                        @method('DELETE')
