...
@section('page_title', 'Editar CTA Session')
    ...
    <div class="flex flex-col lg:flex-row gap-6">
        ...
        <div class="flex-grow max-w-4xl bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 overflow-hidden">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-white flex items-center justify-between">
                <div>
                    <h2 class="font-extrabold text-gray-900 text-lg tracking-tight">Editar CTA Session</h2>
                    <p class="text-xs text-gray-500 mt-1">
                        Atualize os dados e a lista de características da CTA #{{ $cta_session->id }}.
                    </p>
                </div>
            ...
            <form action="{{ route('admin.cta_session.update', $cta_session->id) }}" method="POST" class="p-6 space-y-8">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $cta_session->title) }}"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="subtitle" id="subtitle" rows="3"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition duration-200 bg-gray-50/50 hover:bg-gray-50 focus:bg-white"
                                placeholder="Ex: Entre em contato e planeje sua próxima viagem com especialistas.">{{ old('subtitle', $cta_session->subtitle) }}</textarea>
                        ...
                        <div>
                            ...
                            <select name="page_id" id="page_id"
                                ...
                                @foreach($pages as $page)
                                    <option value="{{ $page->id }}" {{ old('page_id', $cta_session->page_id) == $page->id ? 'selected' : '' }}>
                                        {{ $page->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="order_position" id="order_position" value="{{ old('order_position', $cta_session->order_position) }}" min="0"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition duration-200 bg-gray-50/50 hover:bg-gray-50 focus:bg-white"
                                placeholder="0">
                ...
                {{-- ===== SEÇÃO: ITENS DE LISTA (EDITÁVEIS) ===== --}}
                <div class="bg-slate-50/40 p-6 rounded-2xl border border-slate-100 space-y-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-[11px] text-gray-400 mt-1">Adicione ou edite itens para listar nesta CTA.</p>
                        </div>
                    ...
                    <div id="list-items-container" class="space-y-3">
                        @if($cta_session->cta_session_list)
                            @foreach($cta_session->cta_session_list as $key => $item)
                                <div class="list-item-row rounded-xl border border-slate-100 bg-white shadow-sm overflow-hidden transition-all">
                                    <input type="hidden" name="list_items[{{ $key }}][id]" value="{{ $item->id }}">
                                    <div class="flex items-center justify-between gap-2 px-4 py-2 bg-slate-50 border-b border-slate-100">
                                        <span class="inline-flex items-center gap-2 text-[10px] font-bold text-[#001c3d] uppercase tracking-wider">
                                            <i class="fas fa-circle-check text-[#109e4a]"></i> Item de Lista #{{ $item->order }}
                                        </span>
                                    ...
                                    <div class="p-4 grid grid-cols-1 md:grid-cols-12 gap-3">
                                        <div class="md:col-span-6">
                                            ...
                                            <input type="text" name="list_items[{{ $key }}][title]" value="{{ old('list_items.'.$key.'.title', $item->title) }}" placeholder="Ex: Suporte 24 horas por dia" required
                                        ...
                                        <div class="md:col-span-3">
                                            ...
                                            <div class="flex gap-2 items-center">
                                                <input type="text" name="list_items[{{ $key }}][icon]" value="{{ old('list_items.'.$key.'.icon', $item->icon) }}" placeholder="fa-solid fa-circle-check" required
                                        ...
                                        <div class="md:col-span-2">
                                            ...
                                            <input type="number" name="list_items[{{ $key }}][order]" value="{{ old('list_items.'.$key.'.order', $item->order) }}" required
                                                class="w-full px-3 py-2 rounded-lg border border-slate-200 focus:border-[#001c3d] text-xs focus:outline-none transition-colors">
                                        </div>
                                        <div class="md:col-span-1 flex items-end justify-center pb-2">
                                            <label class="flex items-center gap-1 cursor-pointer select-none">
                                                <input type="hidden" name="list_items[{{ $key }}][active]" value="0">
                                                <input type="checkbox" name="list_items[{{ $key }}][active]" value="1" {{ old('list_items.'.$key.'.active', $item->active) ? 'checked' : '' }}
                ...
                <div class="bg-slate-50/60 p-5 rounded-2xl border border-slate-100">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                        ...
                        <div class="lg:col-span-1">
                            ...
                            <input type="text" name="button_label" id="button_label" value="{{ old('button_label', $cta_session->button_label) }}"
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <input type="text" name="button_url" id="button_url" value="{{ old('button_url', $cta_session->button_url) }}"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition bg-white"
                        ...
                        <div>
                            ...
                            <select name="button_target" id="button_target"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition bg-white">
                                <option value="_self" {{ old('button_target', $cta_session->button_target) == '_self' ? 'selected' : '' }}>Mesma aba</option>
                                <option value="_blank" {{ old('button_target', $cta_session->button_target) == '_blank' ? 'selected' : '' }}>Nova aba</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select name="button_variant" id="button_variant"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition bg-white">
                                <option value="primary" {{ old('button_variant', $cta_session->button_variant) == 'primary' ? 'selected' : '' }}>Primary (Principal)</option>
                                <option value="secondary" {{ old('button_variant', $cta_session->button_variant) == 'secondary' ? 'selected' : '' }}>Secondary (Secundário)</option>
                                <option value="outline" {{ old('button_variant', $cta_session->button_variant) == 'outline' ? 'selected' : '' }}>Outline (Contorno)</option>
                                <option value="ghost" {{ old('button_variant', $cta_session->button_variant) == 'ghost' ? 'selected' : '' }}>Ghost (Transparente)</option>
                            </select>
                        ...
                        <div>
                            ...
                            <div class="flex gap-2 items-center">
                                <input type="text" name="button_icon" id="button_icon" value="{{ old('button_icon', $cta_session->button_icon) }}"
                                ...
                                <span class="button-icon-preview text-gray-400 text-lg w-8 text-center">
                                    @if($cta_session->button_icon)
                                        <i class="{{ $cta_session->button_icon }}"></i>
                                    @else
                ...
                <div class="bg-slate-50/60 p-5 rounded-2xl border border-slate-100">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                        ...
                        <div class="lg:col-span-1">
                            ...
                            <input type="text" name="secondary_button_label" id="secondary_button_label" value="{{ old('secondary_button_label', $cta_session->secondary_button_label) }}"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition bg-white"
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <input type="text" name="secondary_button_url" id="secondary_button_url" value="{{ old('secondary_button_url', $cta_session->secondary_button_url) }}"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition bg-white"
                        ...
                        <div>
                            ...
                            <select name="secondary_button_target" id="secondary_button_target"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition bg-white">
                                <option value="_self" {{ old('secondary_button_target', $cta_session->secondary_button_target) == '_self' ? 'selected' : '' }}>Mesma aba</option>
                                <option value="_blank" {{ old('secondary_button_target', $cta_session->secondary_button_target) == '_blank' ? 'selected' : '' }}>Nova aba</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select name="secondary_button_variant" id="secondary_button_variant"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition bg-white">
                                <option value="secondary" {{ old('secondary_button_variant', $cta_session->secondary_button_variant) == 'secondary' ? 'selected' : '' }}>Secondary (Secundário)</option>
                                <option value="primary" {{ old('secondary_button_variant', $cta_session->secondary_button_variant) == 'primary' ? 'selected' : '' }}>Primary (Principal)</option>
                                <option value="outline" {{ old('secondary_button_variant', $cta_session->secondary_button_variant) == 'outline' ? 'selected' : '' }}>Outline (Contorno)</option>
                                <option value="ghost" {{ old('secondary_button_variant', $cta_session->secondary_button_variant) == 'ghost' ? 'selected' : '' }}>Ghost (Transparente)</option>
                            </select>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                        ...
                        <div>
                            ...
                            <div class="flex gap-2">
                                <input type="color" name="bg_color_picker" id="bg_color_picker" value="{{ old('bg_color', $cta_session->bg_color ?? '#ffffff') }}"
                                ...
                                <input type="text" name="bg_color" id="bg_color" value="{{ old('bg_color', $cta_session->bg_color ?? '#ffffff') }}"
                        ...
                        <div>
                            ...
                            <div class="flex gap-2">
                                <input type="color" name="text_color_picker" id="text_color_picker" value="{{ old('text_color', $cta_session->text_color ?? '#000000') }}"
                                    class="h-11 w-14 rounded-lg border border-gray-200 cursor-pointer p-1"
                                    oninput="document.getElementById('text_color').value = this.value">
                                <input type="text" name="text_color" id="text_color" value="{{ old('text_color', $cta_session->text_color ?? '#000000') }}"
                                    class="flex-1 px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition bg-white"
                                    placeholder="#000000"
                        ...
                        <div>
                            ...
                            <select name="alignment" id="alignment"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition bg-white">
                                <option value="center" {{ old('alignment', $cta_session->alignment) == 'center' ? 'selected' : '' }}>Centralizado</option>
                                <option value="left" {{ old('alignment', $cta_session->alignment) == 'left' ? 'selected' : '' }}>Esquerda</option>
                                <option value="right" {{ old('alignment', $cta_session->alignment) == 'right' ? 'selected' : '' }}>Direita</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="padding_vertical" id="padding_vertical" value="{{ old('padding_vertical', $cta_session->padding_vertical ?? 'py-12') }}"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#001c3d] focus:ring-2 focus:ring-[#001c3d]/10 focus:outline-none text-sm transition bg-white"
                ...
                <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="analytics_event_name" id="analytics_event_name" value="{{ old('analytics_event_name', $cta_session->analytics_event_name) }}"
                ...
                <div class="flex items-center gap-3 bg-slate-50 p-4 rounded-xl border border-slate-100 cursor-pointer select-none hover:bg-slate-100 transition-colors w-fit">
                    <label class="flex items-center gap-3 cursor-pointer">
                        ...
                        <input type="checkbox" name="active" id="active" value="1" {{ old('active', $cta_session->active) ? 'checked' : '' }}
                ...
                <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-100">
                    ...
                    <button type="submit" class="bg-[#109e4a] hover:bg-[#0d8c40] text-white px-8 py-3.5 rounded-xl font-extrabold text-xs uppercase tracking-wider transition-all duration-200 shadow-lg shadow-emerald-500/10">
                        <i class="fas fa-save mr-1.5"></i> Salvar Alterações
                    </button>
                </div>
            </form>
        </div>
...
@section('scripts')
<script>
    let listIndex = {{ $cta_session->cta_session_list ? count($cta_session->cta_session_list) : 0 }};
    ...
    document.getElementById('add-list-item').addEventListener('click', function () {
        ...
        const html = `
            <div class="list-item-row rounded-xl border border-slate-100 bg-white shadow-sm overflow-hidden transition-all">
                <div class="flex items-center justify-between gap-2 px-4 py-2 bg-slate-50 border-b border-slate-100">
                    <span class="inline-flex items-center gap-2 text-[10px] font-bold text-[#001c3d] uppercase tracking-wider">
                        <i class="fas fa-circle-check text-[#109e4a]"></i> Novo Item de Lista #${defaultOrder}
                    </span>
                    <button type="button" class="remove-list-item p-1.5 text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Remover Item">
                        <i class="fas fa-trash-alt text-xs pointer-events-none"></i>
                    </button>
